...
| `looks_like_index_page` | **warning**: sampled lines are mostly wiki links — probably a contents page, not the work (§3) |
...
* It has **never been executed against live data** — the environment it was written in
  cannot reach any corpus host. Treat every "expect" above as a hypothesis to confirm.
